...
streznik.get('/', function(zahteva, odgovor) {
  if (zahteva.session.Username == null || zahteva.session.Username == undefined) odgovor.redirect('/prijava');
  else {
...
streznik.post('/izpisiRacunBaza', function(zahteva, odgovor) {
  ...
  obrazec.parse(zahteva,function(napaka,textFields,datoteke) {
    pesmiIzRacuna(textFields.seznamRacunov, function(pesmi) {
      strankaIzRacuna(textFields.seznamRacunov, function(stranka) {
        odgovor.setHeader('content-type','text/xml');
        odgovor.render('eslog', {
          vizualiziraj:true,
          postavkeRacuna: pesmi,
          customerInfo: stranka
        });
      });
    });
  });
...
var izbranaStranka = function(customerid, callback) {
...
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  pesmiIzKosarice(zahteva, function(pesmi) {
    izbranaStranka(zahteva.session.Username, function(napaka, stranka) {
      if (!pesmi) {
        odgovor.sendStatus(500);
        ...
        odgovor.render('eslog', {
          ...
          postavkeRacuna: pesmi,
          customerInfo: stranka //array podatkov stranke
        });  
      }
    });
  });
